...
$ python run.py $(DIR) --sim -g chips_1200
$ cd $(DIR)/scripts/sim
$ source ../chips_1200_sim.sh
(wait for simulation jobs to finish)
$ cd $(RUN)
$ python run.py $(DIR) --map -g chips_1200
$ cd $(DIR)/scripts/map
$ source ../chips_1200_map.sh
...
class CHIPSRunner:
    ...
    def load_config(self, config_file):
        ...
        env_path = os.getenv("CHIPSENV")
        for key, value in config_dict.items():
            if type(value) != int:
                config_dict[key] = env_path + value
    ...
    def gen_beam(self, ev, par, type):
        ...
        num_jobs = int(ev)/self.config["gen_size"]
        for i in range(num_jobs):
            ...
            if par == "numu":
                spec = self.config["flux_file"] + ",enufullfine_numu_allpar_NoXSec_CHIPSoffAXIS"
                pid = 14
            if par == "anumu":
                spec = self.config["flux_file"] + ",enufullfine_anumu_allpar_NoXSec_CHIPSoffAXIS"
                pid = -14
            elif par == "nuel":
                spec = self.config["flux_file"] + ",enufullfine_nue_allpar_NoXSec_CHIPSoffAXIS"
                pid = 12
            elif par == "anuel":
                spec = self.config["flux_file"] + ",enufullfine_anue_allpar_NoXSec_CHIPSoffAXIS"
                pid = -12
            script.write("\ncd " + path.join(self.dir, "scripts/gen/"))
            script.write("\nmkdir gen" + str(i))
            script.write("\ncd gen" + str(i))
            script.write(
                "\ngevgen -n " + str(self.config["gen_size"]) +
                " -e 0.5,15" +
                " -p " + str(pid) +
                ...
                " -f " + spec +
                " --seed " + str(i) +
                " --cross-sections " + self.config["xsec_file"] +
                " --event-generator-list " + type +
                " --message-thresholds " + os.getenv("GENIE") + "/config/Messenger_laconic.xml"
                " > /dev/null"
            )
    ...
    def gen_cosmic(self, ev, detector, beamdir):
        ...
        num_jobs = int(ev)/self.config["gen_size"]
        for i in range(num_jobs):
            script_name = path.join(self.dir, "scripts/gen/",
                                    detector + "_gen_" + "{:03d}".format(i) + ".sh")
            output_name = "gen_" + "{:03d}".format(i) + ".vec"
            script = self.blank_script(script_name)
            script.write("\nsource " + self.config["setup"])
            script.write("\ncosmicGen " + self.config["cry_config"] + " " +
                         str(self.config["gen_size"]) + " " +
                         detector + " " + str(i))
            if beamdir:
                script.write(" 1 ")
...
def parse_args():
    ...
    parser.add_argument('-t', '--type', help='event list', default='Default+CCMEC+NCMEC')
