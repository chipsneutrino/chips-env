...
//  
  ...
  if      (proc.IsQuasiElastic()    && proc.IsWeakCC()) evtype =  1;
    ...
    else if(proc.IsWeakCC() && init.IsNuN()) {
        // v n -> l- p pi0  
        if(np==1 && nn==0 && npip==0 && npi0==1 && npim==0) evtype = 4;
        // v n -> l- n pi+
        if(np==0 && nn==1 && npip==1 && npi0==0 && npim==0) evtype = 5;
    }
    else if(proc.IsWeakNC() && init.IsNuP()) {
        // v p -> v p pi0
        if(np==1 && nn==0 && npip==0 && npi0==1 && npim==0) evtype = 6;
        // v p -> v n pi+
        if(np==0 && nn==1 && npip==1 && npi0==0 && npim==0) evtype = 7;
    }
    else if(proc.IsWeakNC() && init.IsNuN()) {
        // v n -> v n pi0
        if(np==0 && nn==1 && npip==0 && npi0==1 && npim==0) evtype = 8;
        // v n -> v p pi-
        if(np==1 && nn==0 && npip==0 && npi0==0 && npim==1) evtype = 9;
    }
    else if(proc.IsWeakCC() && init.IsNuBarN()) {
        // \bar{v} n -> l+ n pi-
        if(np==0 && nn==1 && npip==0 && npi0==0 && npim==1) evtype = 10;
    }
    else if(proc.IsWeakCC() && init.IsNuBarP()) {
        // \bar{v} p -> l+ n pi0
        if(np==0 && nn==1 && npip==0 && npi0==1 && npim==0) evtype = 11;
        // \bar{v} p -> l+ p pi-
        if(np==1 && nn==0 && npip==0 && npi0==0 && npim==1) evtype = 12;
    }
    else if(proc.IsWeakNC() && init.IsNuBarP()) {
        // \bar{v} p -> \bar{v} p pi0
        if(np==1 && nn==0 && npip==0 && npi0==1 && npim==0) evtype = 13;
        // \bar{v} p -> \bar{v} n pi+
        if(np==0 && nn==1 && npip==1 && npi0==0 && npim==0) evtype = 14;
    }
    else if(proc.IsWeakNC() && init.IsNuBarN()) {
        // \bar{v} n -> \bar{v} n pi0
